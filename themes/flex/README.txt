...
Flex boasts a clean HTML5 structure with extensible CSS classes and IDs for
...
Flex's goal is to provide themers the building blocks needed to get their
designs up and running quickly and simply.

Flex is perfect if you want a simple, smart, and flexible theme starter.
...
Flex utilizes SASS for adaptive grids and layouts and general structure of the
...
Flex is meant to be YOUR theme. Follow one of the two methods below and you can
rename 'flex' to another name like 'mytheme'. You're also welcome to keep
'flex'.

1. DRUSH: The provided drush install script will duplicate the flex theme
   folder and rename all appropriate files and content to the new theme name you
   provide.

  - Download and enable the flex theme: $ drush en flex
  - Set the theme as default $ drush config-set system.theme default flex
    or copy the includes/flex.drush.inc into your .drush folder.
  - Run the provided drush install script: $ drush flex-install
  ...
  - At this time, you can uninstall flex: $drush pmu flex
...
2. MANUAL: To manually change the name of the theme follow these steps BEFORE
  ...
  - Rename flex.info.yml to mytheme.info.yml
  - edit flex.info.yml and change the name, description, project (can be
    deleted), and change all other instances of 'flex' to 'mytheme'
  - Rename flex.libraries.yml to mytheme.libraries.yml
  - Rename flex.theme to mytheme.theme
  - in flex.theme, change each instance of 'flex' to 'mytheme'
  - Rename config/schema/flex.schema.yml to mytheme.schema.yml
  - Rename each file in config/install from block.block.flex_tools.yml (for
    example) to block.block.mytheme_tools.yml
  - Every file in config/install, change each instance of 'flex' to
    'mytheme'
  - In js/source/scripts.js, change each instance of 'flex' to 'mytheme'
  - In theme-settings.php, change each instance of 'flex' to 'mytheme'
  - In templates/html.html.twig, change each instance of 'flex' to 'mytheme'
  - In templates/menu-local-tasks.html.twig, change each instance of 'flex' to
    'mytheme'
  - In templates/status-messages.html.twig, change each instance of 'flex' to
    'mytheme'

  When renaming, remember the following:
  - Do not simply replace every instance of 'flex' in every file in the theme.
    Most of Flex's dependencies use the word 'flex' somewhere and renaming
    these instances will cause Flex to break in unpredictable ways.
  ...
  - If you don't bother renaming Flex in the above locations, be advised that
    you will run into conflicts with other versions of Flex on your site. If
    your site uses more than one theme based on Flex, make sure at least one of
    the themes has been renamed properly!


============================
How to compile SASS in Flex
============================

To use SASS and automatically compile it within your theme, please refer to "How
to Use Grunt with Flex" in the documentation below.
...
- build/scripts.js & source/scripts.js
  ...
  folder. See comments in flex.libraries.yml file to enable the starter
  scripts.js file.
...
The layout used in Flex is fairly similar to the Holy Grail method. It has been
...
This is how the page template is buit in Flex, and it works in fluid and fixed
layout. Refer to the notes in layout.sass to see how to modify the layout.


===========================
How to Use Grunt with Flex
===========================
...
Once Node.JS is installed, go to the root folder of Flex and install your Grunt
packages:
...
Once installed, cd to the root folder of Flex and run Grunt via the command
line:
...
Once you start using Flex, you will massively change it until you reach the
point where it has nothing to do with Flex anymore. Unlike Zen, Flex is not
...
version of Flex comes out. Always see Flex as a STARTER, and as soon as you
start using it, it is not Flex anymore, but your own theme.

If you didn't rename your theme, but you don't want to be notified when Flex
has a new version by the update module, simply delete "project = "flex" in
flex.info.yml.
...
Thanks for using Flex, and remember to use the issue queue in drupal.org for
any questions or bug reports:

http://drupal.org/project/issues/flex
